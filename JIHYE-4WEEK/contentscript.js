...
// 남은 학점 표시
function displayRemainingCredits(data, completedCourses, remainingCreditsInfo) {
...
fetch(chrome.runtime.getURL('data/informationConvergence.json'))
  ...
  .then(data => {
    ...
    createAndInsertTable(studentInfo, completedCourses); // 테이블 생성 및 삽입

    // 남은 학점 계산 및 표시 로직 실행
    const remainingCredits = calculateRemainingCredits(); // 남은 학점 계산
    console.log(remainingCredits);
    displayRemainingCredits(studentInfo, completedCourses, remainingCredits); // 남은 학점 정보 표시
  })
